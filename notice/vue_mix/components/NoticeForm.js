export default {
    ...
    emits: ['deselect-notice', 'update-list'],
    template: `
      <div id="class-form">
        ...
        <button type="button" class="btn btn-primary mt-3" @click="updateClick">Update</button>
        <button type="button" class="btn btn-danger mt-3" @click="deleteClick">Delete</button>
        <button type="button" class="btn btn-secondary mt-3" @click="updateList">List</button>
      </div>
    `,
    async created() {
      // var _this = this;
    ...
    methods: {
      async updateClick() {
        ...
        // }
        await this.bindModel.cmd['update'].execute();
        if (this.bindModel.cmd['update'].state > 0) this.$emit('deselect-notice');
      },
      async deleteClick() {
        await this.bindModel.cmd['delete'].execute();
        if (this.bindModel.cmd['delete'].state > 0) this.$emit('deselect-notice');
      },
      async readClick(idx) {
        this.bindModel.cmd['read'].outputOption.index = Number(idx);
        await this.bindModel.cmd['read'].execute();
        this.$emit('select-notice', idx);
        // this.$emit('select-notice', 1);
      },
      updateList() {
